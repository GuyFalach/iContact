...
const int ImageProcessing::twoEyesFinded(Frame greyscaleImg, Camera cam, float result[2][3], float originalInfo[2][3], float radius, float minX, float minY, const unsigned int mulColRange, const unsigned int mulRowRange)
{
    if (radius == 0)
    {
        ImageProcessing::contrastEnhancement(greyscaleImg);
    ...
    {
        ImageProcessing::findCircles(cam, originalInfo, result);
    }
...
void ImageProcessing::findCircles(Camera cam, float originalInfo[2][3], float resultBuffer[2][3])
{
    cv::Mat image = cam.giveFrame();
    cvtColor(image, image, cv::COLOR_BGR2GRAY);
    GaussianBlur(image, image, cv::Size(3, 3), 0);
    std::vector<Vec3f> circles;
    std::vector<Vec3f> circleAfterRemove;
    HoughCircles(image, circles, HOUGH_GRADIENT, 2,
            15 , 90, 30, originalInfo[0][RADIUS] - 2, originalInfo[0][RADIUS] + 2);
    for (auto circle : circles)
    {
        if (std::abs(circle[1] - originalInfo[0][Y_AXIS]) < 5 * originalInfo[0][RADIUS])
        {
            if (std::abs(circle[0] - originalInfo[0][X_AXIS]) < 7 * originalInfo[0][RADIUS])
            {
                circleAfterRemove.push_back(circle);
                continue;
            }
        }
    }
    for (auto circle : circles)
    {
        if (std::abs(circle[1] - originalInfo[1][Y_AXIS]) < 5 * originalInfo[1][RADIUS])
        {
            if (std::abs(circle[0] - originalInfo[1][X_AXIS]) < 7 * originalInfo[1][RADIUS])
            {
                if(std::abs(circle[0] - originalInfo[0][X_AXIS]) > 2 * originalInfo[0][RADIUS])
                circleAfterRemove.push_back(circle);
                continue;
            }
        }
    }
    if (circleAfterRemove.size() == 0)
    {
        return;
    }
    resultBuffer[0][X_AXIS] = circleAfterRemove[0][0];
    resultBuffer[0][Y_AXIS] = circleAfterRemove[0][1];
    resultBuffer[0][RADIUS] = circleAfterRemove[0][2];
    if (circleAfterRemove.size() == 1)
    {
        return;
    }
    resultBuffer[1][X_AXIS] = circleAfterRemove[1][0];
    resultBuffer[1][Y_AXIS] = circleAfterRemove[1][1];
}
...
/*
* The function generate the enhanced values the the image should have
*/
void ImageProcessing::contrastValuesGen(Frame originalFrame, unsigned contrastValuesBuffer[PIXEL_MAX_VALUE])
{
    unsigned int totalSize = originalFrame.cols * originalFrame.rows;
    unsigned int pixelsValueCounter[PIXEL_MAX_VALUE] = { 0 };
    float valuesProbability[PIXEL_MAX_VALUE] = { 0 };
    float valuesCDF[PIXEL_MAX_VALUE] = { 0 };

    // the loop count the time that each valuve appear
    for (unsigned int i = 0; i < originalFrame.rows; i++)
    {
        for (unsigned int j = 0; j < originalFrame.cols; j++)
        {
            pixelsValueCounter[originalFrame.frame[i][j]]++;
        }
    }

    //the loop calculate the probablity for each value to appear 
    for (unsigned int i = 0; i < PIXEL_MAX_VALUE; i++)
    {
        valuesProbability[i] = (float)pixelsValueCounter[i] / totalSize;
    }
    valuesCDF[0] = valuesProbability[0];

    //calculate the necessary probability to enhance the image
    for (unsigned int i = 1; i < PIXEL_MAX_VALUE; i++)
    {
        valuesCDF[i] = valuesProbability[i] + valuesCDF[i - 1];
    }

    //calculate the new enhanced value of evert pixel
    for (unsigned int i = 0; i < PIXEL_MAX_VALUE; i++)
    {
        contrastValuesBuffer[i] = round((PIXEL_MAX_VALUE - 1) * valuesCDF[i]);
    }
}


/*
* The function enhances the given image
*/
void ImageProcessing::contrastEnhancement(Frame& originalFrame)
{
    unsigned contrests[PIXEL_MAX_VALUE] = { 0 };
    ImageProcessing::contrastValuesGen(originalFrame, contrests);
    //replace every pixels with its enhanced value
    for (unsigned int i = 0; i < originalFrame.rows; i++)
    {
        for (unsigned int j = 0; j < originalFrame.cols; j++)
        {
            originalFrame.frame[i][j] = contrests[originalFrame.frame[i][j]];
        }
    }
}