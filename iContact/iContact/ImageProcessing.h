...
public:
    ...
    static const int twoEyesFinded(Frame greyscaleImg, Camera cam, float result[2][3], float originalInfo[2][3], float radius = 0, float minX = 0, float minY = 0, const unsigned int mulColRange = 5, const unsigned int mulRowRange = 2);
    ...
    static void contrastEnhancement(Frame& originalFrame);
private:
    static void contrastValuesGen(Frame originalFrame, unsigned contrastValuesBuffer[PIXEL_MAX_VALUE]);
    ...
    static void findCircles(Camera cam, float originalInfo[2][3], float resultBuffer[2][3]);
